...
public class Explosion {
    ...
    public Explosion() throws IOException {
        ...
        new Thread(() -> {
            new MakeSound().playSound("data/vzryv.wav");
        }).start();
    ...
    void paint(Graphics g){
        ...
        if (x != 0) {
            ...
            BufferedImage sc = image.getSubimage(0, 0, image.getWidth(), image.getHeight() / 2);

            g.drawImage(sc, (int) (s.width - 1.0 * s.width / s.numRays * angle - projection / 2), s.height / 2 + (int) (s.height * Math.tan(s.player.vertAngle)), projection, projection / 2, null);
        }
    }
